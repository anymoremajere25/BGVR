...
fn main() -> Result<()> {
    // Initialize logging (e.g., set RUST_LOG=info for verbose output)
    env_logger::init();

    // Parse command-line arguments
    let cli = Cli::parse();
    info!("Starting coverage tool on file: {}", cli.bam);
    ...
    let output_stats = QCStats {
        coverage: total_coverage,
        mismatches: total_mismatches,
    };
    let output_file = format!("qc_{}.json", Path::new(&cli.bam).file_name().unwrap().to_str().unwrap());
    let mut file = File::create(&output_file)
        .with_context(|| format!("Failed to create output file: {}", output_file))?;
    let json = serde_json::to_string(&output_stats)?;
